...
namespace Test
{
    public class OBJ_Logic : Logic
    {
        ...
        public OBJ_Logic(List<Shape> _shapes)
        {
            this.Shapes = _shapes;

            c = new Circle(new System.Numerics.Vector3(0.0f, 0.0f, 0.0f), 0.01f);
        }
        ...
        public override void OnUpdateFrame(FrameEventArgs args, KeyboardState key, MouseState mouse)
        {
            ...
            // c.SetOrigin(new System.Numerics.Vector3(norm_x, norm_y, 0.0f));
            // c.Draw();
            // Shapes.Add(c);
            

            // Shapes.ElementAt(0).SetOrigin(new System.Numerics.Vector3(norm_x, norm_y, 0.0f));

            // Console.WriteLine("{0}:{1}\t\t{2}:{3}", norm_x, norm_y, Shapes.ElementAt(0).GetOrigin().X,  Shapes.ElementAt(0).GetOrigin().Y);
